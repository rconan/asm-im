...
use std::{env, sync::Arc,path::Path,fs::File,collections::HashMap};
use vec_box::vec_box;
use nalgebra as na;
use uid::UID;
...
pub async fn model(cfd_case: &str) -> anyhow::Result<()> {
    ...
    //println!("{state_space}");
        ...
        let signal = {
    let signal: std::result::Result<OneSignal,_> = 
    Signals::new(1,n_step)
    .output_signal(0,Signal::Sigmoid{amplitude:1f64,sampling_frequency_hz:sim_sampling_frequency as f64})
    .progress().into();
            signal?.into_arcx()
};
...
let mount_client = Mount::new().into_arcx();
    ...
    let  m1_hp_loadcells_client =
        m1_ctrl::hp_load_cells::Controller::new().into_arcx();

    let  m1_client_segment1 =
        m1_ctrl::actuators::segment1::Controller::new().into_arcx();
    let  m1_client_segment2 =
        m1_ctrl::actuators::segment2::Controller::new().into_arcx();
    let  m1_client_segment3 =
        m1_ctrl::actuators::segment3::Controller::new().into_arcx();
    let  m1_client_segment4 =
        m1_ctrl::actuators::segment4::Controller::new().into_arcx();
    let  m1_client_segment5 =
        m1_ctrl::actuators::segment5::Controller::new().into_arcx();
    let  m1_client_segment6 =
        m1_ctrl::actuators::segment6::Controller::new().into_arcx();
    let  m1_client_segment7 =
        m1_ctrl::actuators::segment7::Controller::new().into_arcx();
...
// MODEL 
    ...
    let mut cfd_loads: Initiator<_> = Actor::new(cfd_loads_client.clone())
        .name("CFD Wind loads");

    // FEM
    let mut fem: Actor<_> = Actor::new(
        state_space.clone()).name(
        format!(
            "FEM w/ ASM:
        {:}",
    ...
    Actor::new(signal.clone()).name("Sigmoid");
    ...
    let mut sink: Terminator<_> = Arrow::builder(n_step)
            ...
            .decimation(decimation).build().into();
    ...
    let mut mount_set_point: Initiator<_> = Actor::new(mount_set_point_client.clone()).name("Mount Set Point");
    mount_set_point
        .add_output()
        .build::<MountSetPoint>()
    ...
    let mut m1rbm_set_point: Initiator<_> = Actor::new(m1rbm_set_point_client.clone()).name("M1 RBM Set Point");
    m1rbm_set_point
        .add_output()
        .build::<M1RBMcmd>()
    ...
    let mut m2_pos_cmd: Initiator<_> = Actor::new(m2_pos_cmd_client.clone()).name("M2 Positionner Set Point");
    // FSM POSITIONNER
    let mut m2_positionner: Actor<_> = Actor::new(m2_positionner_client.clone()).name("M2 Positionner");
    m2_pos_cmd
        .add_output()
        .build::<M2poscmd>()
