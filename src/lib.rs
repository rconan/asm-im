...
pub async fn model(cfd_case: &str) -> anyhow::Result<()> {
    ...
    for part in (0..n_part).take(1) {
        (*cfd_loads_client.lock().await)
            .start_from(part * n_step / n_part)
            .stop_after((part + 1) * n_step / n_part);
        ...
        let mut sink: Terminator<_> = Arrow::builder(n_step)
            ...
            .filename(format!("asms-im-rbm_windloading-part{part}"))
